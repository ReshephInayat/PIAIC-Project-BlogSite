        ...
        <div className="flex gap-10 flex-wrap">
          {blogs.blogs.map((post: any) => (
            <Link href={`/${post.id}`} key={post.id}>
              <Card className="bg-zinc-800 text-white border-4 border-white hover:scale-110 transition-all duration-300">
                <CardContent className="max-w-xs flex flex-col gap-5 p-4">
                  <Image
                    src={post.photo_url}
                    alt={post.title}
                    width={400}
